...
f = h5py.File("./data_aug_random_iiti_full.h5", 'w')
a_data = np.zeros((len(subjectnames)*21, 25000, 3))
labels=np.zeros((len(subjectnames)*21, 1), dtype = np.uint8)
count = 0
for l in range(0,len(subjectnames)):
	...
	while i<len(facenames):
		c = facenames[i].split('f')[0]
		freq = 0
		
		while i<len(facenames) and facenames[i].split('f')[0]==c:
			freq = freq+1
			i = i+1
		print("freq"+str(freq))
		samples = 0
		j = 1
		while samples < 21:
			s = os.path.join(d_path, facenames[i-j])
			#print(s)
			points = read_asc(s)
			j = (j % freq) + 1
			if (points == None):
