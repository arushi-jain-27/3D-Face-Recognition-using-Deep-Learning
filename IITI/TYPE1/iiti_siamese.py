...
def plot_roc(y_test,y_score):
	...
	plt.title('ROC curve for IITI Dataset')
	plt.legend(loc='best')
	plt.savefig("roc_iiti.png")
...
def plot_far_frr(pred,labels):
        ...
        plt.grid(True)
        plt.savefig("far_frr_iiti_exp.png")
...
def plot_hist(pred, labels):
	...
	bins = 50
	plt.figure(3)
	plt.hist([genuine, imposite],bins, color=["lightseagreen","navy"], log="True")
	plt.xlabel("Predicted probabilities")
	plt.ylabel("Frequency(Log Scale)")
	plt.legend(legend)
	plt.xticks(np.arange(0, 1.1, step=0.1))
	#plt.yticks(np.arange(0, 100))
	plt.title('Genuine Imposite Score Distribution for IITI Dataset')
	plt.savefig("gen_imp_iiti_log.png")
	plt.figure(4)
	plt.hist([genuine, imposite],bins, color=["lightseagreen","navy"])
	plt.xlabel("Predicted probabilities")
...
def plot_cmc(correct,incorrect):
	...
	cmc1 = CMC(cmc_dict)
	cmc1.save(title = 'CMC Curve for IITI Dataset', filename='cmc_iiti_s',
...
with open('iiti_full_train.pkl', 'rb') as f:
	train_points = pickle.load(f)

with open('iiti_full_test.pkl', 'rb') as f:
	test_points = pickle.load(f)

with open('iiti_full_trainl.pkl', 'rb') as f:
	train_labels = pickle.load(f)

with open('iiti_full_testl.pkl', 'rb') as f:
...
x_train, y_train, labels1 = get_data_train(70000, classes,train_points,label_count)
x_test, y_test, labels2 = get_data_train(30000, classes,test_points,label_count_test)
...
out1_val = x_val[:, 0].reshape(x_val.shape[0], 1, 256)
out2_val = x_val[:, 1].reshape(x_val.shape[0], 1, 256)
...
for i in range(1,2):
    if i!=0: 
    	w_name = 'test_trainiiti'
    	model.load_weights(w_name, by_name=True)
    #model.fit([out1, out2], y_train, validation_data=([out1_val, out2_val], y_val), batch_size=4, verbose=1, shuffle=True, epochs=5)
    w_name = 'test_trainiiti'
