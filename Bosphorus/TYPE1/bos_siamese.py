...
def plot_roc(y_test,y_score):
	...
	plt.figure(1)
	plt.plot([0, 1], [0, 1], 'k--')
	plt.plot(fpr_keras, tpr_keras, label='AUC = {:.3f}'.format(auc_keras), color = "red")
	plt.xlabel('False positive rate')
	plt.ylabel('True positive rate')
	plt.title('ROC curve for Bosphorus Neutral Dataset')
	plt.legend(loc='best')
	plt.savefig("roc_bos.png")
...
def plot_far_frr(pred,labels):
        ...
        while i < 1.01:
            ...
            i = i+0.01
        plt.figure(2)
        plt.plot(throld, fpr, color = "deeppink",label="FAR Curve")
        plt.plot(throld, fnr, color = "darkviolet", label="FRR Curve")
        plt.xlabel('Thresholds')
        plt.ylabel('FAR/FRR')
        plt.title('FAR FRR curve for Bosphorus Neutral Dataset')
        plt.legend(loc='best')
        plt.ylim(0,0.015)
        #plt.xlim(-0.001,1.001)
        plt.grid(True)
        plt.savefig("far_frr_bos_s.png")
...
def plot_hist(pred, labels):
	...
	bins = 50
	plt.figure(3)
	plt.hist([genuine, imposite],bins, color=["deeppink","darkviolet"], log="True")
	plt.xlabel("Predicted probabilities")
	plt.ylabel("Frequency(Log Scale)")
	plt.legend(legend)
	plt.xticks(np.arange(0, 1.1, step=0.1))
	#plt.yticks(np.arange(0, 100))
	plt.title('Genuine Imposite Score Distribution for Bosphorus Neutral Dataset')
	plt.savefig("gen_imp_bos_log.png")
	plt.figure(4)
	plt.hist([genuine, imposite],bins, color=["deeppink","darkviolet"])
	plt.xlabel("Predicted probabilities")
...
def plot_cmc(correct,incorrect):
	...
	cmc1 = CMC(cmc_dict)
	cmc1.save(title = 'CMC Curve for Bosphrous Neutral Dataset', filename='cmc_bos_neutral',
         rank=classes,xlabel='Rank Score', ylabel='Recognition Rate', 
...
with open('bos_neutral_train.pkl', 'rb') as f:
	train_points = pickle.load(f)

with open('bos_neutral_test.pkl', 'rb') as f:
	test_points = pickle.load(f)

with open('bos_neutral_trainl.pkl', 'rb') as f:
	train_labels = pickle.load(f)

with open('bos_neutral_testl.pkl', 'rb') as f:
...
def get_data_train(total_sample_size, categories,out,label_count, freq_count):
    ...
    for i in range(categories):
        for j in range(categories):
            if(i!=j):
                ind1 = np.random.randint(label_count[i-1] if i>0 else 0  ,label_count[i]-1)
                ind2 = np.random.randint(label_count[j-1] if j>0 else 0  ,label_count[j]-1)
...
x_test, y_test, labels2 = get_data_train(30000, classes,test_points,label_count_test, freq_count_test)
# In[ ]:
