...
f = h5py.File("./data_aug21_random_bos_neutral_no_split.h5", 'w')
a_data = np.zeros((105*21*4, num_select, 3))
labels = np.zeros((105*21*4, 1), dtype = np.uint8)
count = 0
i = 0
while i<len(samplenames):
	label = int(samplenames[i].split('_')[0].split("bs")[1])
	print(samplenames[i])
	print(label)
	samples = 0
	while samples<21:
